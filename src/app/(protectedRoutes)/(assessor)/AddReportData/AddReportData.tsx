...
const QuillTextArea = dynamic(() => import('@/components/QuillTextArea/QuillTextArea'), { ssr: false });
...
const AddReportData = () => {
  ...
  const handleSave = async () => {
    setIsLoading(true);
    const payload: PayloadType = { tenantId, plantId };
    ...
    try {
      ...
      if (currentQuestionIndex < questions.length - 1) {
        setCurrentQuestionIndex((prev) => {
          const nextIndex = prev + 1;
          contentRef.current = questionContents[nextIndex];
          return nextIndex;
        });
      } else {
        router.push(`/ReportViewPage/${tenantId}/${plantId}`);
  ...
  const handleBack = () => {
    if (currentQuestionIndex > 0) {
      setCurrentQuestionIndex((prev) => {
        const prevIndex = prev - 1;
        contentRef.current = questionContents[prevIndex];
        return prevIndex;
      });
    }
  };
  ...
  return (
    ...
    >
      <Paper
        className={styles.formSection}
        elevation={2}
        sx={{ mt: 2, borderRadius: '16px', backgroundColor: 'white', border: '1px solid rgb(216, 216, 216)' }}
      >
        <Box sx={{ width: '100%', height: '100%', display: 'flex' }} className={styles.bothSections}>
          <Box className={styles.formContainer} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Typography variant="h4" gutterBottom>
